...
function atualizarProdutos(){
    ...
    lista.innerHTML="";
    produtos.forEach(p=>{
        lista.innerHTML+="<li>"+p.nome+" | Estoque: "+p.estoque+" | R$ "+p.preco.toFixed(2)+"</li>";
    });
}

// Pesquisa de produtos
document.getElementById("produtoPesquisa").addEventListener("input", ()=>{
    let query = document.getElementById("produtoPesquisa").value.toLowerCase();
    let listaSugestoes = document.getElementById("listaSugestoes");
    listaSugestoes.innerHTML="";
    produtos.filter(p=>p.nome.toLowerCase().includes(query)).forEach(p=>{
        let li = document.createElement("li");
        li.innerText=p.nome+" | R$ "+p.preco.toFixed(2);
        li.onclick=()=>{
            document.getElementById("produtoPesquisa").value=p.nome;
            listaSugestoes.innerHTML="";
        };
...
function atualizarLista(){
    ...
    carrinho.forEach(item=>{
        lista.innerHTML+="<li>"+item.nome+" | Qtd: "+item.quantidade+" | R$ "+item.valor.toFixed(2)+"</li>";
    });
...
function finalizarVenda(){
    if(carrinho.length===0){ alert("Venda vazia!"); return; }
    ...
    carrinho=[]; totalAtual=0; atualizarLista(); atualizarHistorico();
...
function atualizarHistorico(){
    ...
    vendas.forEach(v=>{
        listaHistorico.innerHTML+="<li>"+v.numero+" - "+v.cliente+" - "+v.data+" - R$ "+v.total.toFixed(2)+"</li>";
    });
}

// PDF
function gerarPDFHistorico(){
    if(vendas.length===0){ alert("Sem vendas!"); return; }
    const { jsPDF } = window.jspdf || {};
    if(!jsPDF){ alert("PDF não suportado!"); return; }
    let doc = new jsPDF();
...
// Caixa
function abrirCaixa(){
    if(caixaAberto){ alert("Caixa já aberto!"); return; }
    let valor = parseFloat(document.getElementById("valorInicial").value);
    if(isNaN(valor)){ alert("Informe o valor inicial"); return; }
    valorCaixa = valor;
    caixaAberto = true;
    document.getElementById("caixaInfo").innerText="Caixa aberto: R$ "+valorCaixa.toFixed(2);
}
function fecharCaixa(){
    if(!caixaAberto){ alert("Caixa não aberto!"); return; }
    let totalVendas = vendas.reduce((a,b)=>a+b.total,0);
    caixaAberto=false;
    document.getElementById("caixaInfo").innerText="Caixa fechado. Total do dia: R$ "+(valorCaixa+totalVendas).toFixed(2)+" em "+new Date().toLocaleString("pt-BR");
}
